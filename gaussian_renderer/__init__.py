...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None,use_norm_mlp=False):
    """
    Render the scene. 
    ...
    sh2norm= torch.zeros_like(pc.get_xyz, dtype=pc.get_xyz.dtype, requires_grad=True, device="cuda") + 0
    try:
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            ...
            if use_norm_mlp:
                ...
                output=sh2norm_norm*pc.norm_mlp2(torch.cat([wr,sh2rgb_normalized,torch.sin(cosine),torch.cos(cosine),torch.tanh(cosine)],dim=1))
